...
public class SeatEntity extends Entity {
    ...
    public static ActionResultType create(World world, BlockPos pos, double yOffset, PlayerEntity player, Direction direction, boolean toilet) {
        if (!world.isClientSide) {
            List<Entity> seats = world.getEntitiesOfClass(SeatEntity.class, new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0, pos.getY() + 1.0, pos.getZ() + 1.0));
            if (seats.isEmpty()) {
                SeatEntity seat = new SeatEntity(world, pos, new Vector3d(0, yOffset, 0),direction);
                seat.toilet = toilet;
                world.addFreshEntity(seat);
                player.startRiding(seat, false);
            }
